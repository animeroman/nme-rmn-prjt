...
function getLives(type = 'all') {
  if (!loading && hasMore) {
    ...
    loading = true;
    $.get(
      `/ajax/watch2gether/list?type=${type}&cursor=${cursor}&status=${room}`,
      function(res) {
        hasMore = res.hasMore;
...
if (foundW2gWatch) {
      ...
      if (diffTime <= 0) {
        ...
        const countdownInterval = setInterval(function() {
          if (diffTime <= 0) {
            ...
            playerWrapper
              .find('#countdown')
              .text(timeFormatBySeconds(diffTime));
          }
        }, interval);
      }
    } else if (airTime === 0) {
  ...
  const messageTmpl = jsrender.templates('#messageTmpl'),
    actionTmpl = jsrender.templates('#actionTmpl');

  const psChat = new PerfectScrollbar('.t_c-chatlist');

  function scrollToLatestMessage() {
    document
      .querySelector('.t_c-chatlist')
      .scrollTo(0, document.querySelector('.t_c-chatlist').scrollHeight);
  }
  ...
  socket.on('receiveMessage', serverData => {
    if (serverData.wait) {
      ...
      document.getElementById('chat-content').innerHTML += messageTmpl.render(
        serverData
      );
      psChat.update();
      if (!$('#chat-scrollBottom').hasClass('show')) {
  ...
  socket.on('initData', svData => {
    if (firstInit) {
      ...
      if (slowMode > 0) {
        ...
        inputMessageEl.attr(
          'placeholder',
          messagePlaceholder + ' (Slow mode is on)'
        );
      }
  ...
  document
    ...
    .addEventListener('mouseover', function(event) {
      ...
      if (timeEl) {
        timeEl.innerHTML = dayjs(
          dayjs.unix(parseInt(timeEl.getAttribute('data-time')))
        ).fromNow();
      }
    });
  ...
  function startChatCountDown() {
    ...
    document.getElementById('countTimer').innerHTML = `${slowMode -
      countTimer}s`;
  ...
  var gifPage = 1,
    giphyApiKey = 'mCHTgpTZWxrnFxaLpVhD44FajzH6JmVs';
      ...
      function(res) {
        if (res.data) {
          res.data.forEach(item => {
            $('#tab-sticker .sticker-list').append(
              `<div class="sl-item"><div class="sli-img"><img src="${item.images.fixed_width_small.webp}"></div></div>`
            );
          });
        }
      }
  ...
  function searchGifs(keyword) {
    const limit = 30,
      offset = (gifPage - 1) * limit;
    $.get(
      `https://api.giphy.com/v1/gifs/search?api_key=${giphyApiKey}&q=${keyword}&limit=${limit}&offset=${offset}&rating=r&lang=en`,
      function(res) {
        if (res.data) {
          res.data.forEach(item => {
            $('#tab-sticker .sticker-list').append(
  ...
  $(document).on('click', '.player-time', function() {
    if ($('#iframe-embed').attr('src') !== '') {
      ...
      document.getElementById('iframe-embed').contentWindow.postMessage(
        JSON.stringify({
          event: 'seek',
          time
        }),
